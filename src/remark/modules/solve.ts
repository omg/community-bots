...
function createSolveRemark(data: WBMRemarkData, solveNumber: number): string {
  let remark = "";
  let winnerSolution = data.currRound.winner.solution;
  let isExactSolve =
    data.currRound.winner.solution === data.currRound.promptWord;
  ...
  if (isExactSolve && !hasRemarkedExactness) {
    remark +=
      ...
      ` **Lucky!** That's your **${formatPlacementWithEnglishWords(
        data.postRoundWinnerData.rankingDocuments["All-Time"].exactSolves
      )} exact solve**!`;
  }

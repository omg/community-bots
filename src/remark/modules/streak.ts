...
export async function execute(data: WBMRemarkData): Promise<string> {
  if (data.streak.consecutiveWins >= 3) {
    ...
    return `**${getRemarkEmoji(solveStreakEmoji)} You're on ${
      isNumberVowelSound(data.streak.consecutiveWins) ? "an" : "a"
    } ${getStreakNumbers(
      data.streak.consecutiveWins
    )} solve streak! ${getRemarkEmoji(solveStreakEmoji)}**`;
  } else if (
    data.streak.consecutiveWins === 1 &&
    data.streak.previous?.consecutiveWins >= 3
  ) {
    return `${getRemarkEmoji("streakEnded")} **${
      data.streak.previous.userDisplayName +
      (data.streak.previous.userDisplayName.endsWith("s") ? "'" : "'s")
    }** solve streak of **${
      data.streak.previous.consecutiveWins
    }** has been ended!`;
  }
}

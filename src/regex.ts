import { SliceException, standardizeWord } from "./dictionary/dictionary";
...
export function getPromptRegexFromPromptSearch(promptQuery: string): RegExp {
  ...
  if (cleanQuery.includes("`")) {
    throw new SliceException("The slice you've entered is invalid.");
  }
  ...
  if (regexResult) {
    ...
    if (regexInput === "") {
      // This regex is empty
      throw new SliceException("The regex you've entered is empty.");
    }
 ...
 * @throws SliceException if the regex is invalid
 */
export function validateRegex(regex: string): RegExp {
  try {
    ...
    throw new SliceException("The regex you've entered is invalid.");
  }
}
...
export function renameRegexGroups(regex: RegExp): RegExp {
  ...
  for (let match of matches) {
    ...
    if (matchString.startsWith("(?<")) {
      ...
      regexString = replaceAt(
        regexString,
        match.index + runningIndexDifferance,
        `(?<${uniqueName}>`
      );
  ...
  for (let backref of regexString.matchAll(/\\([1-9])/gi)) {
    ...
    regexString = regexString.replace(
      backref[0],
      `\\k<${orderedNamedGroups[parseInt(backref[1]) - 1]}>`
    );
  }
...
export function setHighlightGroups(regex: RegExp): RegExp {
  ...
  while (firstIndex !== -1) {
    ...
    let groupedWildcard =
      (regexString[firstIndex - 1] === "(" ||
        regexString[firstIndex - 1] === ">") &&
      regexString[firstIndex + wildcardEnd] === ")";
    if (!groupedWildcard) {
      let groupText = `(?<${HIGHLIGHT_GROUP}${gIndex}>${regexString.slice(
        firstIndex,
        firstIndex + wildcardEnd
      )})`;
      console.log("groupText: ", groupText);
      gIndex++;
      regexString =
        regexString.slice(0, firstIndex) +
        groupText +
        regexString.slice(firstIndex + wildcardEnd);
 ...
 */
export function getHighlightedLetters(
  solution: string,
  regex: RegExp
): Letters[] {
  ...
  let nonHighlightGroups = Object.keys(match.groups || {}).filter((x) =>
    x.startsWith(HIGHLIGHT_GROUP)
  );
  ...
  if (match.index > 0) {
    cutString.push({
      text: solution.slice(0, match.index),
      letterType: "present",
    });
  ...
  if (lastReplacedIndex < match[0].length + match.index) {
    cutString.push({
      text: solution.slice(lastReplacedIndex, match[0].length + match.index),
      letterType: "highlighted",
    });
    lastReplacedIndex = match[0].length + match.index;
  }
  cutString.push({
    text: solution.slice(lastReplacedIndex),
    letterType: "present",
  });
 ...
 * ```
 */
export function getPromptRegexDisplayText(
  regex: RegExp,
  ...
  if (!invalidPromptDisplayRegex.test(displayString)) {
    return highlighter
      ? highlighter.getPresent(displayString, true)
      : regexString;
  }

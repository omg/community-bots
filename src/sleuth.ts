...
function updatePresence() {
  let memberCount = sleuthClient.guilds.cache.get(
...
export async function createBoosterRole(
  ...
  let userBoosterRole = await sleuthClient.guilds.cache
...
export async function renameRole(roleID: string, newName: string) {
  await sleuthClient.guilds.cache
    .get(process.env.GUILD_ID)
...
/**
 * Set the icon and name of a role using Sleuth.
 *
 * **IMPORTANT NOTES:**
 * This function does not resize the provided icon.
 * This function does not check if the server has reached Boost Level 2.
 * This function does not check if the name is valid or clashes with other roles.
 *
 * It is unknown if the icon must be resized before being passed to this function, but it is highly recommended.
 * A Discord role icon must be a PNG, JPEG, or WEBP image with a maximum size of 256 KB.
 *
 ...
 * @param name The name of the role
 */
export async function editRole(roleID: string, icon: Buffer, name: string) {
  await sleuthClient.guilds.cache
    .get(process.env.GUILD_ID)
    .roles.cache.get(roleID)
    .edit({

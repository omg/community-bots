...
export class WordBombMini extends TextChannelBasedGame {
  ...
  async compileRemarkData(): Promise<PostRoundUserData> {
    ...
    let rankingDocument = await getUserRankingInfo<WBMRankingDocument>(
      winner.user
    );
    ...
    rankingDocument.forEach((doc) => {
      let lb_name = this.active_leaderboards.get(
        doc.leaderboardID.toHexString()
      );
    ...
    let rankingAfter;
    if (rankingBefore) {
      rankingAfter = await getUserRanking(winner.user);
    }
  ...
  async sendRoundMessage() {
    let m =
      ...
      "\n\n" +
      getPromptRegexDisplayText(this.currentRound.prompt) +
      " ***｡✲ﾟ** (" +
      formatNumber(this.currentRound.solutionCount) +
      (this.currentRound.solutionCount === 1 ? " solution)" : " solutions)") +
      (this.currentRound.lengthRequired
        ? "\n\n• Must be **" +
          this.currentRound.promptWord.length +
          "** characters!"
        : "");
  ...
  async endRound() {
    ...
    await sendMessage(
      ...
      `**${getRemarkEmoji("solvedIt")} <@${
        this.currentRound.winner.user
      }> solved it! ${getRemarkEmoji("solvedIt")}**\n\n` +
        getRemarkEmoji("roundEnded") +
        " **Round ended!**\n" +
        convertTextToHighlights(
          this.currentRound.winner.solution,
          this.currentRound.prompt
        ) +
        "\n" +
        remarks
    );
      ...
      if (round.lengthRequired && guess.length !== round.promptWordLength) {
        ...
        message.reply(
          "<@" +
            ...
            "** characters!\nYours was **" +
            guess.length +
            (guess.length < round.promptWordLength
              ? "**, go higher " + getRemarkEmoji("higher")
              : "** go lower " + getRemarkEmoji("lower"))
        );
      }

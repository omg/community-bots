...
// string | Message, i dont wanna fix issues
// caused by type hinting right now
let replyMessage;
...
// mose of these definitions dont need to be typed, but this one especially does
let solves: Array<{ user: string, solution: string, usedVivi: boolean }> = [];

function isNumberVowelSound(x: number): boolean {
  return x == 11 || x == 18 || x.toString().startsWith("8");
}

function getCurrentPromptName(): string {
  return (
...
function addRemark({ index, remark = "" }): void {
  if (remarks[index]) {
...
// TODO: terrible name
function formatPlacementWithEnglishWords(x: number): string {
  return NUMBER_WORDS[x] || formatPlacement(x);
}

// TODO: terrible names?
// maybe typehint these as any instead?
function engNum(x: number, singular, plural): string {
  return x === 1 ? singular : plural;
}
function engLen(x: string[], singular, plural): string {

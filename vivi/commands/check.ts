...
export async function execute(
  ...
  const unavailableEmoji = isHomeServer
    ? "<:Unavailable:1267171561987637248>"
    : "<:Unavailable:1267171142632738961>";
  const availableEmoji = isHomeServer
    ? "<:Available:1267171548712669286>"
    : "<:Available:1267171118251376731>";
  ...
  if (word.length > 34) {
    await replyToInteraction(
      ...
      "\n• **" +
        ...
        "\n" +
        unavailableEmoji +
        " Too long** to be a valid English word.",
      preferBroadcast
    );
  } else if (isWord(word)) {
    await replyToInteraction(
